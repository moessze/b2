...
import { makeListenerGuard } from './listener-guard.js';
...
export const makeEventBuffer = (logger) => {
    ...
    // sock.ev is where the consumer's own handlers live - message handlers, plugin code, whatever
    // the host wired up. An async one of those that rejects is an unhandled rejection, and Node
    // kills the process over it, taking every other account in the host down with the one that
    // failed. Guarded at registration so a listener added by anyone gets the same protection.
    const listenerGuard = makeListenerGuard(err => logger.error({ err }, 'unhandled error in event listener'));
    const trackedOn = (event, handler) => {
        const guarded = listenerGuard.wrap(handler);
        ev.on(event, guarded);
        trackedListeners.push([event, guarded]);
    };
    trackedOn('event', (map) => {
    ...
    return {
        ...
        // unwrap, or removal would look for the caller's function while the registration holds the
        // guarded wrapper - every off() would silently leave the listener attached.
        off: (event, handler) => ev.off(event, listenerGuard.unwrap(handler)),
