...
export const generateWAMessageContent = async (message, options) => {
    ...
    // Lia@Changes 09-04-26 --- Add support for code block and table with richResponseMessage
        ...
        if (options.textColorArgb) {
            extContent.textArgb = await assertColor(options.textColorArgb);
        }
        m.extendedTextMessage = extContent;
    }
    else if (hasNonNullishProperty(message, 'groupStatusMessage') ||
        hasNonNullishProperty(message, 'groupStatusMessageV2')) {
        const inner = hasNonNullishProperty(message, 'groupStatusMessageV2')
            ? message.groupStatusMessageV2
            : message.groupStatusMessage;
        const innerMessage = inner?.message || inner;
        if (!innerMessage || typeof innerMessage !== 'object') {
            throw new Boom('Invalid group status message', { statusCode: 400 }); // Lia@Note 08-09-26 --- To avoid bug 👀
        }
        const { backgroundColor, font, color, ...contentBody } = innerMessage;
        if (backgroundColor) options.backgroundColor = backgroundColor;
        if (font !== undefined) options.font = font;
        if (color) options.textColorArgb = color;
        m = await generateWAMessageContent(contentBody, options);
        if (!hasOptionalProperty(m, 'groupStatusMessageV2')) {
            message.groupStatus = true; // Lia@Note 08-09-26 --- let the wrap below normalize it into groupStatusMessageV2
        }
    }
    // Lia@Changes 08-09-26 --- Accept nested group status content { groupStatus: { text|image|video|audio } }
    else if (hasNonNullishProperty(message, 'groupStatus') &&
        message.groupStatus &&
        typeof message.groupStatus === 'object') {
        const { backgroundColor, font, color, ...contentBody } = message.groupStatus;
        if (backgroundColor) options.backgroundColor = backgroundColor;
        if (font !== undefined) options.font = font;
        if (color) options.textColorArgb = color;
        m = await generateWAMessageContent(contentBody, options);
        if (!hasOptionalProperty(m, 'groupStatusMessageV2')) {
            message.groupStatus = true;
        }
    }
