...
export const fetchLatestBaileysVersion = async (options = {}) => {
    // upstream's own version feed, kept current on the update-version/stable branch
    const URL = 'https://raw.githubusercontent.com/WhiskeySockets/Baileys/refs/heads/update-version/stable/src/Defaults/baileys-version.json';
    try {
        ...
        // a truncated or reshaped payload must fall back, not be handed to the handshake
        if (!Array.isArray(version) || version.length !== 3 || version.some(n => !Number.isInteger(n))) {
...
/** Trim and upcase a pairing code: the phone upcases what the user types before deriving its key. */
export const normalizePairingCode = (code) => String(code ?? '').trim().toUpperCase();
/**
 * Characters in a custom pairing code that fall outside the alphabet WhatsApp draws link codes
 * from. 0, I, O and U are absent from it on purpose - they are the glyphs a person mistypes - so
 * a code containing them is one the phone may normalize before deriving its key, while we derive
 * ours from the literal string. The keys then differ, the primary device cannot decrypt our
 * wrapped ephemeral key, and the user sees "Couldn't link device" with nothing logged either side.
 *
 * Whether WhatsApp really normalizes is not something this library can observe, so this only
 * reports; the caller warns. Note L IS in this alphabet even though standard Crockford drops it,
 * so it must never be folded into 1.
 */
export const invalidPairingCodeChars = (code) => [
    ...new Set([...normalizePairingCode(code)].filter(c => !CROCKFORD_CHARACTERS.includes(c)))
];
/** The characters WhatsApp's own generated pairing codes are built from. */
