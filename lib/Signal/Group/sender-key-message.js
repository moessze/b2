...
import { calculateSignature as fastCalculateSignature, verifySignature as fastVerifySignature } from '../xeddsa-fast.js';

const calculateSignature = (privateKey, message) => {
    try {
        return fastCalculateSignature(privateKey, message);
    }
    catch (_) {
        return libCalculateSignature(privateKey, message);
    }
};
const verifySignature = (publicKey, message, signature) => {
    try {
        return fastVerifySignature(publicKey, message, signature);
    }
    catch (_) {
        return libVerifySignature(publicKey, message, signature);
    }
};
