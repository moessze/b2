...
const detectMediaTypeFromBuffer = (buffer) => {
    if (buffer.length > 3 &&
    ...
    if (buffer.length > 3 &&
        buffer.toString('latin1', 0, 4) === 'OggS') {
        return 'audio';
    }
    if (buffer.length > 2 &&
...
const detectMediaTypeFromString = (value) => {
    const pathname = value.split(/[?#]/)[0].toLowerCase();
    if (/\.(png|jpe?g|gif|webp|bmp)$/.test(pathname)) return 'image';
    if (/\.(mp4|mkv|mov|3gp|webm|avi)$/.test(pathname)) return 'video';
    if (/\.(mp3|m4a|aac|wav|opus|ogg|amr)$/.test(pathname)) return 'audio';
    return 'image';
};
const getMediaTypeForGroupStatus = (media) => {
    if (!media) return null;
    if (Buffer.isBuffer(media)) {
        return detectMediaTypeFromBuffer(media);
    }
    if (typeof media === 'string') {
        return detectMediaTypeFromString(media);
    }
    if (media && typeof media === 'object' && typeof media.url === 'string') {
        return detectMediaTypeFromString(media.url);
    }
    return null;
};
export const makeMessagesSocket = (config) => {
    ...
    /**
     * Echo a message we just sent back into our own store, off the send path.
     *
     * It runs detached on purpose - the caller has already handed the message to the server and
     * must not wait on bookkeeping - which is exactly why it needs its own catch: a rejection from
     * a detached async callback is an unhandled rejection, and Node kills the process over it,
     * disconnecting every other account the host is running.
     */
    const emitOwnUpsert = (msg) => {
        messageMutex
            .mutex(() => upsertMessage(msg, 'append'))
            .catch((err) => logger.error({ err, key: msg?.key }, 'failed to upsert own message'));
    };
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, addBizAttributes, statusJidList, statusPrivacy }) => {
        ...
        // Lia@Changes 29-09-26 --- Group status messages do NOT carry a meta attr (aligned with moessze/baileyz)
        const isPollUpdate = innerMessage?.pollUpdateMessage;
        if (isNeedMetaAttrs || isPollUpdate) {
            const metaAttrs = {};
            if (isNeedMetaAttrs) {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (Array.isArray(jid)) {
                ...
                if (config.emitOwnEvents) {
                    setImmediate(() => emitOwnUpsert(fullMsg));
                }
                for (const id of jid) {
                    ...
                    if (config.emitOwnEvents) {
                        process.nextTick(() => emitOwnUpsert(mentionMsg));
                    }
                    await delay(delayMs);
                }
                return fullMsg;
                ...
                if ('album' in content) {
                    ...
                    for (const albumMedia of content.album) {
                        ...
                        if (config.emitOwnEvents) {
                            process.nextTick(() => emitOwnUpsert(albumMsg));
                        }
                        await delay(delayMs);
                    }
                }
        ...
        // Lia@Changes 08-09-26 --- Add "groupStatus" socket method to post a status/story into a group
        // Lia@Note 08-09-26 --- Content: { text, background|color } | { media, caption, background } | { image|video|audio, caption }
        groupStatus: async (jid, content = {}, options = {}) => {
            const { background, color, media, caption, ...rest } = content;
            const sendOptions = { ...options };
            if (background) {
                sendOptions.backgroundColor = background;
            }
            if (color) {
                sendOptions.textColorArgb = color;
            }
            if (media && !rest.image && !rest.video && !rest.audio && !rest.sticker) {
                const mediaType = getMediaTypeForGroupStatus(media);
                if (!mediaType || !['image', 'video', 'audio'].includes(mediaType)) {
                    throw new Boom('Invalid media type for group status, expected image, video or audio', { statusCode: 400 });
                }
                rest[mediaType] = typeof media === 'string'
                    ? { url: media }
                    : media;
                if (mediaType === 'audio' && rest.ptt === undefined) {
                    rest.ptt = true;
                }
            }
            if (caption !== undefined && (rest.image || rest.video || rest.audio)) {
                rest.caption = caption;
            }
            return this.sendMessage(jid, { ...rest, groupStatus: true }, sendOptions);
        }
    };
};
