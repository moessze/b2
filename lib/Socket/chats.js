...
export const makeChatsSocket = (config) => {
    ...
    const sendPresenceUpdate = async (type, toJid) => {
        const me = authState.creds.me;
        // `me` is absent until pairing completes, and a presence update is one of the first things a
        // host sends on connect. Reading me.name off undefined throws a TypeError out of an async
        // path most callers do not await, which is an unhandled rejection and a dead process.
        if (!me) {
            logger.warn({ type }, 'not authenticated yet, ignoring presence update request...');
            return;
        }
        ...
        if (isAvailableType || type === 'unavailable') {
            ...
            // Every chatstate type needs a target. jidDecode returns undefined for a missing or
            // malformed jid, and destructuring that throws - so a caller that forgot the second
            // argument gets a clear refusal instead of a crash on a shared host.
            if (!toJid) {
                logger.warn({ type }, 'no jid given for chatstate presence update, ignoring...');
                return;
            }
            const decoded = jidDecode(toJid);
            if (!decoded) {
                logger.warn({ type, toJid }, 'could not decode jid for chatstate presence update, ignoring...');
                return;
            }
