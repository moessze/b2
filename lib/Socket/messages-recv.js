...
export const makeMessagesRecvSocket = (config) => {
    ...
    const requestPlaceholderResend = async (messageKey, msgData) => {
        ...
        setTimeout(async () => {
            // A rejection from a bare async timer callback is unhandled, and Node kills the process
            // on those - taking every other session in it down with this one.
            try {
                if (await placeholderResendCache.get(messageKey?.id)) {
                ...
                logger.error({ err, messageKey }, 'placeholder resend cleanup failed');
            }
        }, 8000);
        return sendPeerDataOperationMessage(pdoMessage);
    };
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
            ...
            case 'link_code_companion_reg':
                ...
                // passkey-locked accounts can emit this notice without the pairing payload (upstream
                // #2681). toRequiredBuffer throws on it, and the throw propagates out of
                // handleNotification and takes the pairing attempt down with it.
    ...
    async function decipherLinkPublicKey(data) {
        // TextEncoder().encode(undefined) is the 9 bytes of the word "undefined", so a missing code
        // silently derives a perfectly valid key over the wrong input: pairing then fails on the
        // phone with nothing in our logs to point at.
        if (!authState.creds.pairingCode) {
            throw new Boom('Received a link code registration but no pairing code is stored locally', { statusCode: 400 });
        }
